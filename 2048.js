window.onload = function() {
  ...
  /* MOVE TILES */
    ...
    // Aggiungi il gestore di eventi per lo scroll
    window.addEventListener('wheel', handleScroll);
  
    // Funzione per gestire lo scroll del mouse
    function handleScroll(e) {
        e.preventDefault(); // Previene il comportamento di scroll predefinito della pagina
        
        // Determina la direzione dello scroll
        if (e.deltaY < 0) { // Scroll verso l'alto
            moveTiles('up');
        } else if (e.deltaY > 0) { // Scroll verso il basso
            moveTiles('down');
        } else if (e.deltaX < 0) { // Scroll verso sinistra
            moveTiles('left');
        } else if (e.deltaX > 0) { // Scroll verso destra
            moveTiles('right');
        }
    }
  
    // Funzione per spostare le tessere in base alla direzione
